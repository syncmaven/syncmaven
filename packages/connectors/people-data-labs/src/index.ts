import { z } from "zod";
import { EnrichmentProvider, rpc, RpcError, stdProtocol } from "@syncmaven/node-cdk";
      ...
      enrichRow: async row => {
        ...
        if (cached) {
          ...
          if (new Date(expire) > new Date()) {
            ...
            console.debug(`Record for ${email} expired at ${expire}, cache key=${cacheKey.join("::")}`);
          }
        }
        if (!response && !isTruish(config.options.cacheOnly)) {
          try {
            if (rateLimitExpires && rateLimitExpires > new Date()) {
              ...
              response = await rpc(`https://api.peopledatalabs.com/v5/person/enrich?email=${email}&api_key=${apiKey}`);
            }
          } catch (e: any) {
            if (e instanceof RpcError) {
              if (e.statusCode === 404) {
                ...
                console.warn(`Rate limit exceeded while fetching PDL data for ${email}. See headers`, e.headers);
                ...
                console.error(`Error fetching PDL data for ${email}. Status code ${e.statusCode}: ${e.message}`);
                return [row];
              }
            } else {
              console.error(`Error fetching PDL data for ${email}: ${e.message}`);
              return [row];
            }
          }
