...
export type RpcParams<Query = Record<string, any>, Payload = any> = Omit<
  RequestInit,
  "method" | "body"
> & {
  method?: string;
...
export const rpc: RpcFunc = async (url, { body, ...rest } = {}) => {
  const urlWithQuery = notEmpty(rest?.query)
    ? urlWithQueryString(url, rest?.query || {})
    : url;
  ...
  try {
    ...
    throw new RpcError(
      `Error calling ${method} ${url}: ${e?.message ? `: ${e.message}` : ""}`,
      {
        statusCode: -1,
        url: urlWithQuery,
        headers: {},
  ...
  if (
    ...
    (result.headers.get("Content-Type") ?? "").startsWith(
