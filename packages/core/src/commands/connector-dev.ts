...
export async function connectorDev(
  ...
  const dir = path.resolve(
    path.join(connectorDirectory || opts.connectorDir || process.cwd()),
  );
  configureEnvVars([".env", ".env.local"], dir, opts.env || []);
  const packageJsonPath = path.join(dir, "package.json");
  if (!fs.existsSync(packageJsonPath)) {
    throw new Error(
      `Connector directory ${dir} does not contain package.json: ${packageJsonPath}`,
    );
  }
  ...
  const connection = readProjectObjectFromFile(
    opts.connectionFile,
    ConnectionDefinition,
    { ignoreDisabled: true },
  );
  ...
  const sync: { id: string; factory: Factory<SyncDefinition> } = looksLikePath(
    opts.syncFileOrId,
  )
  ...
  console.log(
    `Running sync ${sync.id} with model ${model.id} and connection ${connection.id}`,
    project,
  );
