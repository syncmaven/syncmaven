...
export async function connectorDev(connectorDirectory: string | undefined, opts: { connectorDirectory?: string, modelFile: string, syncFileOrId: string, connectionFile: string, env?: string[] }) {
  const dir = path.join(connectorDirectory || opts.connectorDirectory || process.cwd());
  configureEnvVars([".env", ".env.local"], dir, opts.env || [])
  const packageJsonPath = path.join(dir, "package.json");
  if (!fs.existsSync(packageJsonPath)) {
    throw new Error(`Connector directory ${dir} does not contain package.json`);
  }
  ...
  const project: Project = {
    ...
    connection: {
      [connection.id]: connection.factory,
    },
  };
